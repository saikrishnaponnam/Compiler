...
Cname        : ID       		{Cptr = CInstall($1->varname,NULL);printf("Class installed\n");}         
...
AsgStmt:	ID '=' expr ';'		{
		...
		| FIELD '=' NEW '(' ID ')' ';' 		{$$=createtree(0,nodetype_NEW,typevoid,NULL,$1,$5,NULL);}
...
FIELD : FIELD '.' ID 		{	//right most of Field can't be a class variable it must be a type variable
	  ...
	  |	ID '.' ID 			{	//will not occur for class variables ID1 can't be class variable
	  							Ltemp=LLookup($1->varname);
									//printf("A1\n");
								if(Ltemp==NULL)
								{
...
FieldFunction 	: SELF '.' ID '(' ArgList ')' 	{	//Occurs only in class function
                ...
                | ID '.' ID '(' ArgList ')'  	{	//This will not occur inside a class.
													...
													if(Ltemp==NULL)
													{
														...
														{	
															...
															Ctemp=tempst->cptr;
															$1->cptr=Ctemp;
															Mtemp=CMLookup(Ctemp,$3->varname);
															if(Mtemp==NULL)
                ...
                | FIELD '.' ID '(' ArgList ')'  {
                									if($1->cptr==NULL)
                										{yyerror("Invalid Refernce");exit(1);}
                									Mtemp=CMLookup($1->cptr,$3->varname);
                									if(Mtemp==NULL)
                										{yyerror("Invalid Member Function");exit(1);}
                									$3->type=Mtemp->type;
                									$$=createtree(0,nodetype_FIELDFUNC,$3->type,NULL,$1,$3,$5);
...
Fdef :	TypeName ID '('	{functype=type;}  ParamList ')' '{' LDeclarations Body '}' {
																...
																//printf("f0\n");
																validateparams(functype,Cptr,$2->varname,Phead);
																$2->Lentry=Lhead;
																//printf("f1\n");
																if(Cptr==NULL)
																{
