...
void fillVFT(FILE *fp)
{
	...
	while(Ctemp!=NULL)
	{
		...
		i=4096+(Ctemp->class_index)*8;
		while(Mtemp!=NULL)
		{
			fprintf(fp,"MOV [%d],F%d\n",i+Mtemp->funcposition,Mtemp->flabel);
			Mtemp=Mtemp->next;
		}
		Ctemp=Ctemp->next;
	}
...
//ASSINGNMENT
		case nodetype_ASGN:
			...
			if(t->left->cptr!=NULL)
			{
				if(t->Lentry==NULL)				//ID = ID
				{
					i=getreg();	
					fprintf(fp,"MOV R%d,[%d]\n",i,t->right->Gentry->binding);
					fprintf(fp,"MOV [%d],R%d\n",t->left->Gentry->binding,i);
					fprintf(fp,"MOV R%d,[%d]\n",i,(t->right->Gentry->binding)+1);
					fprintf(fp,"MOV [%d],R%d\n",(t->left->Gentry->binding)+1,i);
					freereg();
				}
				else							//SELF.ID = SELF.ID
				{
					i=getadrs(t->right,fp);
					j=getadrs(t->left,fp);
					k=getreg();
					fprintf(fp,"MOV R%d,[R%d]\n",k,i);
					fprintf(fp,"MOV [R%d],R%d\n",j,k);
...
//Functions
		case nodetype_funcall:
			...
			else
			{
				...
				{
					i=getreg();
					Mtemp=MLookup(t->end->cptr,t->left->varname,t->right);
					if(strcmp(t->end->varname,"self")==0)
					{
						fprintf(fp, "MOV R%d,%d\n",i,t->end->Lentry->binding);
						k=getreg();
				...
				{
					Mtemp=MLookup(t->end->right->cptr,t->left->varname,t->right);
					j=getadrs(t->end,fp);
					fprintf(fp, "ADD R%d,1\n",j);
					fprintf(fp, "MOV R%d,[R%d]\n",j,j);
					fprintf(fp, "ADD R%d,%d\n",j,Mtemp->funcposition);
					fprintf(fp, "MOV R%d,[R%d]\n",i,i);
					fprintf(fp, "CALL R%d\n",j);
					freereg();
				}
		...
		case nodetype_NEW:
			...
			if(t->left->nodetype==nodetype_ID)
			{
				...
				{
					fprintf(fp,"MOV [%d],R%d\n",t->left->Gentry->binding,i);
					fprintf(fp,"MOV [%d],%d\n",t->left->Gentry->binding+1,4096+(t->right->cptr->class_index)*8);
				}
				else
				{
					//printf("n2\n");
					...
					fprintf(fp, "MOV [R%d],R%d\n",j,i);
					fprintf(fp, "ADD R%d,1\n",j);
					fprintf(fp, "MOV [R%d],%d\n",j,4096+(t->right->cptr->class_index)*8);
					freereg();
				}
				freereg();
			}
